...
const SHOP = [
  ...
  {name:"Música: Horizonte",type:"music",price:2000,rarity:"MÚSICA",icon:"🎵"},
  {name:"Música: Cosmos",type:"music",price:2500,rarity:"MÚSICA",icon:"🎶"},
  {name:"Música: Aventura",type:"music",price:3000,rarity:"MÚSICA",icon:"🎼"}
];
...
const CUSTOM_SHOP = Array.isArray(ASSET_OVERRIDES.customItems)?ASSET_OVERRIDES.customItems:[];
if(CUSTOM_SHOP.length) SHOP.push(...CUSTOM_SHOP);
...
let audioCtx=null;
let onlineId=cryptoRandomId();
let accountToken=localStorage.getItem("knowWorldToken")||"";
...
function sync(){
  ...
  const readyDaily=DAILY_MISSIONS.filter(m=>{const st=state.dailyStats;const v=m[3]==="categories"?st.categories.length:(st[m[3]]||0);return v>=m[4]&&!state.daily.claimed[m[0]]}).length;
  const readyWeekly=WEEKLY_MISSIONS.filter(m=>{const st=state.weeklyStats;const v=m[3]==="categories"?st.categories.length:(st[m[3]]||0);return v>=m[4]&&!state.weekly.claimed[m[0]]}).length;
  $("#missionBadge")?.classList.toggle("hidden",readyDaily+readyWeekly===0);
  ...
  set("#soundBtn",state.sound?"🔊":"🔇");set("#volumeValue",`${Math.round((state.volume??AUDIO.volume??.55)*100)}%`);if($("#volumeSlider"))$("#volumeSlider").value=Math.round((state.volume??AUDIO.volume??.55)*100);
...
$$("[data-ranking-tab]").forEach(b=>b.addEventListener("click",async()=>{$$("[data-ranking-tab]").forEach(x=>x.classList.remove("active"));b.classList.add("active");if(b.dataset.rankingTab==="online")await loadOnlineRanking();else renderRanking(ranking().all);}));
...
function equipItem(name){const item=itemByName(name);if(!item||!state.purchased.includes(name))return;if(item.type==="title")state.equippedTitle=name.replace(/^Título /,"");else state.equippedItems[item.type]=name;save();sync();saveCloud();if(item.type==="music"){stopBackgroundMusic();startBackgroundMusic();}else playSound("equip");toast(`✓ ${name} equipado. ¡Ahora puedes verlo en el juego!`);}
function unequipItem(name){const item=itemByName(name);if(!item)return;if(item.type==="title")state.equippedTitle="";else state.equippedItems[item.type]="";if(item.type==="music")stopBackgroundMusic();save();sync();toast(`✓ ${name} retirado.`);}
...
function updateTitleSelect(){const select=$("#titleSelect");if(!select)return;const current=select.value;const av=unlockedTitles();select.innerHTML='<option value="">Sin título</option>'+av.map(t=>`<option value="${escapeAttr(t.name)}">${escapeHtml(t.name)}${state.purchased.includes("Título "+t.name)?" · TIENDA":""}</option>`).join("");const modalOpen=!$("#profileModal")?.classList.contains("hidden");select.value=modalOpen && [...select.options].some(o=>o.value===current)?current:(state.equippedTitle||"");$("#previewName").textContent=state.name||"TU NOMBRE";$("#previewTitle").textContent=select.value||"Sin título";}
...
function loadQuestion(){
  ...
  timeLeft=20;$("#timer").textContent=timeLeft;timer=setInterval(()=>{timeLeft--;$("#timer").textContent=timeLeft;if(timeLeft<=0){clearInterval(timer);if(!answeredCurrent)answer(-1,null,q,true);}},1000);
}
...
function answer(index,button,q,timeout){
  ...
  if(correct)state.answeredTotal++;updateStats(correct,q,xpGain,coinGain);save();sync();saveCloud();
  if(state.lives>0)$("#nextBtn").classList.remove('hidden');else showNoLives();
}

function buyLifeWithCoins(){if(state.lives>=5)return toast('Ya tienes 5 vidas.');if(state.coins<500)return toast('Necesitas 500 monedas.');state.coins-=500;state.lives++;if(state.lives>=5)state.lastLifeRecovery=Date.now();save();sync();saveCloud();playSound('coin');if(!$("#quizView").classList.contains('hidden'))loadQuestion();toast('❤️ Vida comprada por 500 monedas.');}
function buyLifeWithXp(){if(state.lives>=5)return toast('Ya tienes 5 vidas.');const cost=xpLifeCost();if(state.xp<cost)return toast(`Necesitas ${fmt(cost)} EXP.`);state.xp-=cost;state.lives++;state.xpLifePurchaseCount++;if(state.lives>=5)state.lastLifeRecovery=Date.now();save();sync();saveCloud();if(!$("#quizView").classList.contains('hidden'))loadQuestion();toast(`❤️ Vida comprada por ${fmt(cost)} EXP. Precio se reinicia a 100 cada 24 h.`);}
...
function musicLibrary(){return AUDIO.musicLibrary||{};}
function currentMusicUrl(){const name=state.equippedItems?.music||AUDIO.defaultMusic||"";const item=itemByName(name);return item?.audio||musicLibrary()[name]||AUDIO.music||"";}
function startBackgroundMusic(){
  if(!state.sound)return;
  const url=currentMusicUrl();if(!url)return;
  try{let a=audioPlayers.music;if(!a||a.src!==new URL(url,location.href).href){a=audioPlayers.music=new Audio(url);a.loop=true;}a.volume=Number(state.musicVolume??AUDIO.musicVolume??.18);a.play().then(()=>musicStarted=true).catch(()=>{});}catch{}
}
function stopBackgroundMusic(){const a=audioPlayers.music;if(a){a.pause();a.currentTime=0;}musicStarted=false;}
function playSound(kind){if(!state.sound)return;startBackgroundMusic();if(playAudioUrl(kind))return;try{audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();const osc=audioCtx.createOscillator(),gain=audioCtx.createGain(),now=audioCtx.currentTime;const map={click:[520,.06],correct:[760,.16],wrong:[180,.18],coin:[900,.09],reward:[1100,.2],equip:[620,.12]};const [freq,dur]=map[kind]||map.click;osc.frequency.value=freq;osc.type=kind==='wrong'?'sawtooth':'sine';gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.07,now+.01);gain.gain.exponentialRampToValueAtTime(.0001,now+dur);osc.connect(gain);gain.connect(audioCtx.destination);osc.start(now);osc.stop(now+dur+.02);}catch{}}
function setMusic(name){state.equippedItems.music=name||"";musicStarted=false;stopBackgroundMusic();save();sync();startBackgroundMusic();}
function setVolume(v){state.volume=Number(v);state.musicVolume=Number(v)*.45;save();sync();Object.values(audioPlayers).forEach(a=>{if(a)a.volume=a===audioPlayers.music?state.musicVolume:state.volume});}

async function api(path,opts={}){try{const r=await fetch(path,{headers:{'Content-Type':'application/json',...(accountToken?{Authorization:'Bearer '+accountToken}:{})},...opts});return await r.json()}catch(e){return {error:'No se pudo conectar con el servidor.'}}}
async function saveCloud(){if(!accountToken)return;await api('/api/sync',{method:'POST',body:JSON.stringify({state})});}
async function loadOnlineRanking(){const d=await api('/api/ranking');if(d.rows)renderOnlineRanking(d.rows);}
function renderOnlineRanking(rows){const box=$("#rankingRows");if(!box)return;box.innerHTML=rows.slice(0,50).map((p,i)=>`<div class="rank-row ${p.name===state.name?'me':''}"><b>${i+1}</b><span>${escapeHtml(p.name)}${p.name===state.name?' — TÚ':''}</span><strong>${fmt(p.xp)} EXP</strong></div>`).join('')||'<p class="empty-owned">Todavía no hay jugadores verificados.</p>';}
async function heartbeat(){const d=await api('/api/online',{method:'POST',body:JSON.stringify({id:onlineId})});if(d.count!=null)set('#onlineCount',`${d.count} conectados`)}
function showAccount(){$("#accountModal")?.classList.remove('hidden');}


function renderCustomShop(){const grid=$(".shop-grid");if(!grid)return;CUSTOM_SHOP.forEach(item=>{if(grid.querySelector(`[data-item="${CSS.escape(item.name)}"]`))return;const card=document.createElement('article');card.className='item panel';card.dataset.item=item.name;card.innerHTML=`<div>${item.icon||'🎁'}</div><h3>${escapeHtml(item.name)}</h3><span>${escapeHtml(item.rarity||'ESPECIAL')}</span><b>🪙 ${fmt(item.price||0)}</b><button data-buy data-name="${escapeAttr(item.name)}" data-price="${Number(item.price||0)}">COMPRAR</button>`;grid.appendChild(card);card.querySelector('[data-buy]').onclick=()=>buyShopItem(item.name,Number(item.price||0));});}
function buyShopItem(name,price){if(state.purchased.includes(name))return toast('Ya tienes este artículo.');if(state.coins<price)return toast(`Necesitas ${fmt(price)} monedas.`);state.coins-=price;state.purchased.push(name);save();sync();saveCloud();playSound('coin');toast(`🎁 ${name} adquirido. Ve a Perfil para USARLO.`);}
...
$("#soundBtn")?.addEventListener('click',()=>{state.sound=!state.sound;save();sync();if(state.sound)playSound('click');else stopBackgroundMusic();});
$("#volumeSlider")?.addEventListener('input',e=>setVolume(e.target.value/100));
$("#accountBtn")?.addEventListener('click',showAccount);
$("#registerBtn")?.addEventListener('click',async()=>{const d=await api('/api/register',{method:'POST',body:JSON.stringify({username:$("#accountUser").value.trim(),password:$("#accountPass").value,email:$("#accountEmail").value.trim()})});if(d.devCode)$("#verificationCode").value=d.devCode;toast(d.error||d.message||'Cuenta creada.');});
$("#verifyBtn")?.addEventListener('click',async()=>{const d=await api('/api/verify',{method:'POST',body:JSON.stringify({username:$("#accountUser").value.trim(),code:$("#verificationCode").value.trim()})});toast(d.error||'Correo verificado. Ya puedes iniciar sesión.');});
$("#loginBtn")?.addEventListener('click',async()=>{const d=await api('/api/login',{method:'POST',body:JSON.stringify({username:$("#accountUser").value.trim(),password:$("#accountPass").value})});if(d.token){accountToken=d.token;localStorage.setItem('knowWorldToken',accountToken);if(d.state){state={...freshState(),...d.state,equippedItems:{...freshState().equippedItems,...(d.state.equippedItems||{})}};save();sync()}toast('✓ Sesión iniciada y progreso sincronizado.')}else toast(d.error||'No se pudo iniciar sesión.');});
$("#saveCloudBtn")?.addEventListener('click',async()=>{await saveCloud();toast('☁️ Progreso guardado en tu cuenta.')});
$("#resetRequestBtn")?.addEventListener('click',async()=>{const d=await api('/api/reset/request',{method:'POST',body:JSON.stringify({email:$("#resetEmail").value.trim()})});if(d.devCode)$("#resetCode").value=d.devCode;toast(d.error||d.message||'Código enviado.');});
$("#resetConfirmBtn")?.addEventListener('click',async()=>{const d=await api('/api/reset/confirm',{method:'POST',body:JSON.stringify({email:$("#resetEmail").value.trim(),code:$("#resetCode").value.trim(),newPassword:$("#newPassword").value})});toast(d.error||'✓ Contraseña cambiada.');});
$("#changeUsernameBtn")?.addEventListener('click',async()=>{const d=await api('/api/change-username',{method:'POST',body:JSON.stringify({email:$("#resetEmail").value.trim(),code:$("#resetCode").value.trim(),newUsername:$("#newUsername").value.trim()})});toast(d.error||'✓ Usuario cambiado. Inicia sesión de nuevo.');});
$("#shareProgressBtn")?.addEventListener('click',shareProgress);
$("#timeHelp")?.addEventListener('click',()=>{if(state.coins<2)return toast('No tienes suficientes monedas.');if(answeredCurrent)return toast('La pregunta ya terminó.');state.coins-=2;timeLeft+=5;$("#timer").textContent=timeLeft;save();sync();playSound('click');toast('⏱ +5 segundos · 🪙 2');});
$("#removeHelp")?.addEventListener('click',()=>{if(state.coins<8)return toast('No tienes suficientes monedas.');if(answeredCurrent)return toast('La pregunta ya terminó.');const q=JSON.parse(JSON.stringify(state.lastQuestionId?questions.find(x=>x.id===state.lastQuestionId):null));if(!q)return;state.coins-=8;$$('.option').filter(b=>Number(b.dataset.index)!==q.answer).slice(0,2).forEach(b=>{b.style.opacity='.2';b.disabled=true;});save();sync();playSound('click');toast('🎯 Se eliminaron 2 opciones · 🪙 8');});

$$('[data-buy]').forEach(btn=>btn.addEventListener('click',()=>buyShopItem(btn.dataset.name,Number(btn.dataset.price))));
...
async function shareProgress(){
  ...
  x.fillStyle='#06152f';x.fillRect(0,0,c.width,c.height);x.fillStyle='#19b9ff';x.font='900 54px Arial';x.fillText('KNOW WORLD',70,90);x.fillStyle='#fff';x.font='700 38px Arial';x.fillText(state.name||'Jugador',70,160);x.font='900 30px Arial';x.fillText(`Nivel ${levelForXp(state.xp)} · ${fmt(state.xp)} EXP`,70,220);x.fillText(`Rango #${ranking().pos}`,70,275);x.fillText(`🔥 Mejor racha: ${state.bestStreak}`,70,330);x.fillText(`❤️ Vidas: ${state.lives}/5`,70,385);x.fillText(`🏅 ${state.equippedTitle||'Sin título'}`,70,440);x.font='24px Arial';x.fillStyle='#8fdcff';x.fillText('Explorador del saber',70,510);const blob=await new Promise(r=>c.toBlob(r,'image/png'));const file=new File([blob],'know-world-progreso.png',{type:'image/png'});if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({title:'Mi progreso en KNOW WORLD',files:[file]});}else{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='know-world-progreso.png';a.click();URL.revokeObjectURL(a.href);toast('🖼️ Imagen de progreso creada.');}}
...
setInterval(()=>{if($("#rankingView")&&!$("#rankingView").classList.contains("hidden"))loadOnlineRanking()},15000);
(async function init(){initStatsIfNeeded();ensureMissions();await loadQuestions();renderCategoryChoices();renderCustomShop();if(state.volume==null)state.volume=AUDIO.volume??.55;if(state.musicVolume==null)state.musicVolume=AUDIO.musicVolume??.18;$("#volumeSlider")&&( $("#volumeSlider").value=Math.round(state.volume*100) );sync();heartbeat();if(!state.name){$("#nameModal")?.classList.remove('hidden');setTimeout(()=>$("#firstName")?.focus(),120);}else $("#nameModal")?.classList.add('hidden');})();
